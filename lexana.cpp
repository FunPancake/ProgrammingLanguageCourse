...
enum TokenType { 
	KEYWORD, IDENTIFIER, NUMBER, SYMBOL, OPERATOR,UNKNOWN 
	};
struct Token {
	string value;
	TokenType type;
...
string keywords[] = {
	...
	"case", "break", "continue", "return", "void", 
	"const", "static", "struct", "class", "public", 
	"private", "protected", "enum", "typedef", "sizeof", 
...
vector<Token> tokenize(const string &code) {
	...
	for (size_t i = 0; i < code.size(); i++) {
		...
		if (isdigit(ch)) {
			temp = "";
			while (i < code.size() && (isdigit(code[i]) || code[i] == '.')) temp += code[i++]; i--; tokens.push_back({temp, NUMBER});
			}
			
		//check if alphabet
		else if (isalpha(ch) || ch == '_') { 
			temp = "";
			while (i < code.size() && (isalnum(code[i]) || code[i] == '_')) temp += code[i++]; i--; tokens.push_back({temp, classify(temp)});
			}
